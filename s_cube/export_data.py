...
class DataWriter:
    ...
    def __init__(self, face_ids: pt.Tensor, nodes: pt.Tensor, centers: pt.Tensor, levels: pt.Tensor, save_dir: str,
                 save_name: str = "data_final_grid", grid_name: str = "final grid",
                 times: list = None) -> None:
        """
        TODO: for refactoring the datawriter:
                - convert write_times to list[str] and check if consistent with entries in HDF5 prior writing to HDF5
                - maybe i need to add a check if the number of cells of the given field match the one of the original
                  grid to avoid interpolation issues
