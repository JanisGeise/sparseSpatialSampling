...
class Dataloader:
    ...
    """
    def __init__(self, load_path: str, file_name: str):
        """
        implements a DataLoader class for conveniently loading the HDF5 file generated by S^3

        :param load_path: path to the HDF5 file and its file name, e.g. '../test_file.h5'
        :type load_path: str
        """
        ...
        self._vertices = None   # TODO: rename to cell centers -> consistent with S^3 (but not with flowtorch)
        self._faces = None
        self._nodes = None      # TODO vertices inconsistent with S^3 -> rename everything in S^3 to nodes or ok?
    ...
    def nodes(self) -> pt.Tensor:
        """
        load the cell vertices of the grid

        :return: cell vertices of the grid
        :rtype: pt.Tensor
        """
    ...
    def load_snapshots(self, field_name: str, write_times: Union[str, List[str]] = None,
                       dtype: pt.dtype = pt.float32) -> pt.Tensor:
        """
        load the snapshots for the given field and time steps
        ...
        :type dtype: pt.dtype
        :return: data matrix containing the snapshots as: [N_cells, N_dimensions, N_snapshots] (vector field) and
                 [N_cells, N_snapshots] (scalar field)
        :rtype: pt.Tensor
        """
        ...
        # check if vector field or scalar field and allocate the data matrix accordingly
        _file = File(join(self._load_path, self._file_name), "r")
        shape = _file.get(f"{DATA}/{write_times[0]}/{field_name}_center")[()].shape

        if len(shape) == 1:
            _data_matrix = pt.zeros((self._n_cells, len(write_times)), dtype=dtype)
        else:
            _data_matrix = pt.zeros((shape[0], shape[1], len(write_times)), dtype=dtype)

        # assemble the data matrix for the given time steps
        for i, k in enumerate(write_times):
            if len(shape) == 1:
                _data_matrix[:, i] = pt.from_numpy(_file.get(f"{DATA}/{k}/{field_name}_center")[()])
            else:
                _data_matrix[:, :, i] = pt.from_numpy(_file.get(f"{DATA}/{k}/{field_name}_center")[()])
