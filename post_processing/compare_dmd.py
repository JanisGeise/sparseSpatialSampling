...
if __name__ == "__main__":
    ...
    ax[2][1].tricontourf(interpolated_field["coordinates"][:, 0], interpolated_field["coordinates"][:, 1],
                         ...
                         levels=levels)
    fig.colorbar(tcf1, ax=ax[0][1], shrink=0.75, label=r"$p / p_{\infty}$")
    fig.colorbar(tcf2, ax=ax[1][1], shrink=0.75, label=r"$p / p_{\infty}$")
    fig.colorbar(tcf3, ax=ax[2][1], shrink=0.75, label=r"$p / p_{\infty}$")
    ...
    fig.tight_layout()
    fig.subplots_adjust()
    plt.savefig(join(save_path_results, f"comparison_reconstructed_fields_dmd_rank_{rank}_t_" +
    ...
    fig.tight_layout()
    fig.subplots_adjust()
    plt.savefig(join(save_path_results, f"comparison_dmd_modes_rank_{rank}.png"), dpi=340)
    plt.close("all")
