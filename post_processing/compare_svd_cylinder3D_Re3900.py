...
def plot_pod_modes(coord_original, coord_inter, U_orig, U_inter, _save_path: str, _save_name: str, n_mode: int = 4,
                   d: float = 0.1, plane: int = 0) -> None:
    """
    ...
    vmax = max(U_orig[:, :n_mode].max().item(), U_inter[:, :n_mode].max().item())
    vmin = -vmax
    ...
    if plane == 0:
        ...
        fig, ax = plt.subplots(n_mode, 2, sharex="all", sharey="all", figsize=(6, 3))
    else:
        raise NotImplementedError

    for row in range(n_mode):
        ax[row][0].tricontourf(coord_original[:, 0] / d, coord_original[:, 1] / d, U_orig[:, row], vmin=vmin,
                               vmax=vmax, levels=levels, cmap="seismic", extend="both")
        # flip sign of modes so it is consistent
        if 1 < row < n_mode:
            ax[row][1].tricontourf(coord_inter[:, 0] / d, coord_inter[:, 1] / d, U_inter[:, row] * -1,
                                   vmin=vmin, vmax=vmax, levels=levels, cmap="seismic", extend="both")
        else:
            ax[row][1].tricontourf(coord_inter[:, 0] / d, coord_inter[:, 1] / d, U_inter[:, row], vmin=vmin,
                                   vmax=vmax, levels=levels, cmap="seismic", extend="both")
        if plane == 0:
...
if __name__ == "__main__":
    ...
    legend = ["$original$"] + [r"$\mathcal{M} = " + f"{m}$" for m in ["0.27", "0.53", "0.77"]]
    ...
    save_path_results = join("..", "run", "final_benchmarks", "cylinder3D_Re3900_local_TKE",
                             "plots_SVD_with_geometry_refinement_no_dl_constraint_500_snapshots")
    ...
    for case in range(len(metric)):
        ...
        for cmp in range(3):
            plot_pod_modes(vertices_orig[:, :2], vertices_inter[:, :2], modes_orig[:, cmp, :].squeeze(),
                           modes_inter[:, cmp, :].squeeze(), save_path_results,
