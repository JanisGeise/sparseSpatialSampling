...
def plot_pod_modes(coord_original, coord_inter, U_orig, U_inter, _s_orig, _s_inter, _save_path: str, _save_name: str,
                   n_modes: int = 4, _geometry: list = None, chord: float = 0.15) -> None:
    ...
    :param n_modes: number of modes to plot
    :param _geometry: airfoil geometry
    ...
    vmax = max(U_orig[:, :n_modes*2].max().item(), U_inter[:, :n_modes*2].max().item())
    vmin = -vmax
    ...
    fig, ax = plt.subplots(n_modes, 2, sharex="all", sharey="all", figsize=(6, 7))
    for row, m in enumerate(range(0, n_modes*2, 2)):
        ax[row][0].tricontourf(coord_original[:, 0] / chord, coord_original[:, 1] / chord, U_orig[:, m], vmin=vmin,
                               vmax=vmax, levels=levels, cmap="seismic", extend="both")
        # flip sign of modes so it is consistent
        if row in flip:
            ax[row][1].tricontourf(coord_inter[:, 0] / chord, coord_inter[:, 1] / chord, U_inter[:, m] * -1,
                                   vmin=vmin, vmax=vmax, levels=levels, cmap="seismic", extend="both")
        else:
            ax[row][1].tricontourf(coord_inter[:, 0] / chord, coord_inter[:, 1] / chord, U_inter[:, m], vmin=vmin,
                                   vmax=vmax, levels=levels, cmap="seismic", extend="both")
        ...
        # add value of the corresponding singular value as title
        for col in range(2):
            if col == 0:
...
def plot_mode_coefficients(write_times: pt.Tensor, V: list, _save_path: str, _save_name: str, legend: list,
    ...
    # plot every 2nd mode, since they are pair-wise
    for row, m in enumerate(range(0, n_modes*2, 2)):
        for i, v in enumerate(V):
            if row == 0:
                ax[row].plot(write_times[:v.shape[0]], v[:, m], color=color[i], label=legend[i], ls=ls[i])
            else:
