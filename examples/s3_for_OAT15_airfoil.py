...
if __name__ == "__main__":
    ...
    field_name = "Ma"
    area = "large"
    save_path = join("..", "run", "final_benchmarks", "OAT15_large_new", "results_with_geometry_refinement_no_dl_constraint")
    ...
    # load the corresponding write times, the field is available for all time steps so we can pass it to directly
    ...
    for v in min_variance:
        ...
        s_cube = SparseSpatialSampling(xz, metric, geometry, save_path, save_name, "OAT15", min_metric=v,
        ...
        # loop over multiple fields (rho, p, ma, u) and export
        for f in [0, "rho", "p", "u"]:
            if f == 0:
                export.export(xz, field.unsqueeze(1), field_name)
                continue
            else:
                field = pt.load(join("..", "data", "2D", "OAT15", f"{f}_{area}_every10.pt"))

            # we need to add one dimension if we have a scalar field
            if len(field.size()) == 2:
                export.export(xz, field.unsqueeze(1), f)
            else:
                export.export(xz, field, f)

        # compute SVD on grid generated by S^3 and export the results to HDF5 & XDMF
        write_svd_s_cube_to_file(field_name, save_path, save_name, export.new_file, 50, rank=int(1e5))
